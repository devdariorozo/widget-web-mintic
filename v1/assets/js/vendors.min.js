    ...
    var c = function() {
        ...
        (o = [{
            ...
            value: function(t, e, n) {
                ...
                var currentPath = window.location.pathname.split('/').slice(1, -1)
                    .map(function(segment) {
                        return segment.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
                    })
                    .join('/');
        
                // Determinar el prefijo de ruta y si debe omitirse la ruta actual
                var routePrefix = '';
                var omitPath = false;
                if (currentPath === 'login') {
                    ...
                    routePrefix = 'auth';
                    omitPath = true;
                } else if (currentPath === 'actualizar-contrasena') {
                    routePrefix = 'home';
                    omitPath = true;
                ...
                // Construir la nueva URL con el prefijo correspondiente
                var newPath = routePrefix 
                    ? (omitPath ? `/languages/${routePrefix}/${t[0]}.json` : `/languages/${routePrefix}/${currentPath}/${t[0]}.json`)
                    : `/languages/${currentPath}/${t[0]}.json`;
        
                var i = this.services.interpolator.interpolate(newPath, {
                    lng: t.join("+"),
                    ns: e.join("+")
                });
                // console.log('ruta final', i);
                this.loadUrl(i, n)
            }
        }, {
            key: "read",
            value: function(t, e, n) {
                ...
                var currentPath = window.location.pathname.split('/').slice(1)
                    .map(function(segment) {
                        return segment.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
                    })
                    .join('/');
                ...
                // Construir la nueva URL con el prefijo correspondiente
                var newPath = routePrefix 
                    ? (omitPath ? `/languages/${routePrefix}/${t}.json` : `/languages/${routePrefix}/${currentPath}/${t}.json`)
                    : `/languages/${currentPath}/${t}.json`;
